...
pub async fn top_volume(limit: usize) -> Result<(), Box<dyn Error>> {
    let json = fetch_raydium_data(1).await?;
    let pools = json["data"]["data"]
        .as_array()
        .ok_or("No pools data found")?;

    println!("Top {} tokens by volume:", limit);
    for (i, pool) in pools.iter().take(limit).enumerate() {
        let symbol = pool["mintB"]["symbol"].as_str().ok_or("No symbol found")?;
        let volume = pool["day"]["volume"]
            .as_f64()
            .ok_or("No day volume found")?;
        println!("{}. {}: ${:.2}", i + 1, symbol, volume);
    }

    Ok(())
}

pub async fn token_info(symbol: &str) -> anyhow::Result<()> {
    let mut page = 1;
    loop {
        let json = fetch_raydium_data(page).await?;
        let pools = json["data"]["data"]
            .as_array()
            .ok_or(anyhow::anyhow!("No pools data found"))?;
        ...
        for pool in pools {
            if pool["mintB"]["symbol"].as_str() == Some(symbol) {
                println!("Information for token {}:", symbol);
                println!("Price: {}", pool["price"].as_f64().unwrap());
                println!(
                    "24h Volume: ${:.2}",
                    pool["day"]["volume"].as_f64().unwrap()
                );
                println!("TVL: ${:.2}", pool["tvl"].as_f64().unwrap());
                return Ok(());
            }
        }
